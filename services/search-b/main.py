...
app.middleware("http")(
    require_payment(
        ...
        path="/api/search",
        network=NETWORK,
        description="Web search results (up to 5 sources) - provider B",
...
if __name__ == "__main__":
    ...
    print(f"[{PROVIDER_NAME}] listening on :{PORTS['search_b']} (payTo {PAY_TO}, price {PRICE})")
    uvicorn.run(app, host="0.0.0.0", port=PORTS["search_b"])
